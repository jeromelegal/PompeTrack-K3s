...
from app.telegram.formatters import format_alerts, format_review_result_brief
...
async def notify_soft_alerts(alerts: list[dict[str, Any]]) -> None:
    settings = get_settings()
    if not alerts:
        return
    if not settings.telegram_bot_token:
        logger.warning("telegram alert skipped: TELEGRAM_BOT_TOKEN is missing")
        return
    ...
    if not chat_ids:
        logger.warning("telegram alert skipped: no notify chat id is configured")
        return

    client = TelegramClient(settings.telegram_bot_token)
    text = format_alerts({"alerts": alerts})
    for chat_id in chat_ids:
        try:
            client.send_message(chat_id, text, disable_notification=True)
        except Exception as exc:  # noqa: BLE001
            logger.warning("failed to send telegram soft alert", extra={"chat_id": chat_id, "error": str(exc)})
