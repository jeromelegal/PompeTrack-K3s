...
def _ingest_bytes(data: bytes, bucket: str, device: dict, object_name: str | None = None):
    ...
    response = upload_file(data, bucket, object_name=object_name)
    if not response.get("ok"):
        raise HTTPException(status_code=500, detail=response.get("error", "upload failed"))
    return {"status": "ok", "bytes": len(data), "device": device.get("device")}
...
async def ingest_iphone(
    payload: dict = Body(...),
    device: dict = Depends(require_scopes(["ingest:iphone"])),
):
...
async def ingest_sqlite(
    ...
    device: dict = Depends(require_scopes(["ingest:sqlite"])),
):
...
async def ingest_fhir(
    payload: dict = Body(...),
    device: dict = Depends(require_scopes(["ingest:fhir"])),
):
...
async def ingest_generic(
    ...
    device: dict = Depends(require_scopes(["ingest:generic"])),
):
    try:
        data = await file.read()
...
async def ingest_dataframe(
    file: UploadFile = File(...),
    device: dict = Depends(require_scopes(["ingest:df"])),
):
    data = await file.read()
...
async def download_object(
    bucket: str,
    object_name: str,
    device: dict = Depends(require_scopes(["download:object"])),
):
...
async def get_json_object(
    bucket: str,
    object_name: str,
    device: dict = Depends(require_scopes(["download:json"])),
):
    try:
...
async def bucket_creation(
    bucket: str,
    device: dict = Depends(require_scopes(["bucket:create"])),
):
...
async def bucket_object_list(
    bucket: str,
    device: dict = Depends(require_scopes(["object:list"])),
):
...
async def move_object_endpoint(
    ...
    device: dict = Depends(require_scopes(["object:move"])),
):
...
async def delete_object_endpoint(
    bucket_name: str,
    object_name: str,
    device: dict = Depends(require_scopes(["object:delete"])),
):
    if not object_delete(object_name, bucket_name):
