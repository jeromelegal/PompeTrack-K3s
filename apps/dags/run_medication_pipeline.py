...
with DAG(
    dag_id="medication_minio_watch_and_run",
    ...
    tags=["medication", "minio", "ingestion"],
) as dag:
    ...
    get_worker_headers_task = MedplumHeaderOperator(
        ...
        scope=["airflow:medication"],
    )

    run_worker = HttpOperator(
        task_id="run_worker_medication",
        http_conn_id="worker_fhir",
        endpoint="/run/medication",
        method="GET",
