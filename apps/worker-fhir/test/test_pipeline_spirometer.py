...
def mock_external(monkeypatch):
    ...
    """
    from pipeline_spirometer import get_object_list, get_object_json, move_object
    ...
    monkeypatch.setattr('pipeline_spirometer.get_object_list', mocks['get_object_list'])
    monkeypatch.setattr('pipeline_spirometer.get_object_json', mocks['get_object_json'])
    monkeypatch.setattr('pipeline_spirometer.move_object', mocks['move_object'])
    monkeypatch.setattr('pipeline_spirometer.pipeline_metrics', mocks['pipeline_metrics'])

    return mocks

# Tests for `spirometer_json_pipeline`   
def test_pipeline_success(mock_external):
    ...
    mock_external['get_object_list'].assert_called_once_with(bucket='raw-spirometer')
    mock_external['get_object_json'].assert_called_once_with(bucket='raw-spirometer', object_name='file1.json')
    
    # Verify `move_object` called once
    mock_external['move_object'].assert_called_once_with(
        object_name='file1.json',
        source_bucket='raw-spirometer',
