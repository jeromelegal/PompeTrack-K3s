...
def normalize_value(value: Optional[Union[str, float, int]]) -> str:
    ...
    - None -> ""
    - float -> normalized decimal string
...
def to_fhir_datetime(value: Union[str, datetime]) -> str:
    ...
    """
    if value is None:
        raise ValueError("value ne peut pas être None pour to_fhir_datetime().")
    ...
    if isinstance(value, str):
        ...
        raise TypeError(
            f"value doit être str ou datetime pour to_fhir_datetime(), reçu {type(value)}"
        )
...
def build_observation_hash(
    ...
    """
    Build a deterministic hash for an observation identifier.
    """
    if not isinstance(patient_id, str):
        raise TypeError(f"patient_id doit être str, reçu {type(patient_id)}")
    if not isinstance(measurement_type, str):
        raise TypeError(f"measurement_type doit être str, reçu {type(measurement_type)}")
