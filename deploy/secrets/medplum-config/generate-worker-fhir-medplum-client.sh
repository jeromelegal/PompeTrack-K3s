...
# =========================================================
command -v curl >/dev/null 2>&1 || { echo "ERROR: curl manquant" >&2; exit 1; }
command -v jq   >/dev/null 2>&1 || { echo "ERROR: jq manquant" >&2; exit 1; }

[[ -f "$DEVICES_FILE" ]] || { echo "ERROR: fichier introuvable: $DEVICES_FILE" >&2; exit 1; }

# Vérifie que le JSON est valide et que c'est bien un objet
jq -e 'type == "object"' "$DEVICES_FILE" >/dev/null || {
  echo "ERROR: $DEVICES_FILE doit contenir un objet JSON" >&2
  exit 1
}
...
## Device
ensure_device() {
  local identifier_system="$1"
  local identifier_value="$2"
  local display="$3"
  local manufacturer="${4:-}"
  local model="${5:-}"

  echo "==> Ensure Device exists (identifier=${identifier_value})" >&2

  local device_q
  device_q="$(jq -rn \
    --arg v "${identifier_system}|${identifier_value}" \
    '$v|@uri'
  )"

  local device_id
  device_id="$(
    curl -fsS "${FHIR_BASE}/Device?identifier=${device_q}&_count=1" \
      -H "$AUTHZ_HEADER" \
  ...
  if [[ -z "${device_id:-}" ]]; then
    echo "==> Device not found, creating" >&2

    local payload
    payload="$(jq -n \
      --arg sys   "$identifier_system" \
      --arg val   "$identifier_value" \
      --arg disp  "$display" \
      --arg man   "$manufacturer" \
      --arg model "$model" \
      '
      {
        resourceType: "Device",
      ...
      + (if $model != "" then { modelNumber:  $model } else {} end)
      '
    )"

    local device_json
    device_json="$(
      curl -fsS "${FHIR_BASE}/Device" \
        -X POST \
        -H "$AUTHZ_HEADER" \
        -H 'Content-Type: application/fhir+json' \
        -d "$payload"
  ...
  if [[ -z "${device_id:-}" ]]; then
    echo "ERROR: impossible de déterminer l'ID du Device '${identifier_value}'" >&2
    return 1
  fi
...
{
  echo "# Generated by generate-worker-fhir-medplum-client.sh"
  echo "MEDPLUM_PROJECT_ID=${PROJECT_ID}"
  echo "MEDPLUM_PATIENT_ID=${PATIENT_ID}"
} > "$tmp_out"

# Lecture des devices depuis le JSON
while IFS= read -r row; do
  key="$(jq -r '.key' <<< "$row")"
  identifier_system="$(jq -r --arg def "$DEVICE_IDENTIFIER_SYSTEM_DEFAULT" '.value.identifier_system // $def' <<< "$row")"
  identifier_value="$(jq -r '.value.identifier_value // empty' <<< "$row")"
  display="$(jq -r '.value.display // empty' <<< "$row")"
  manufacturer="$(jq -r '.value.manufacturer // ""' <<< "$row")"
  model="$(jq -r '.value.model // ""' <<< "$row")"

  if [[ -z "$identifier_value" ]]; then
    echo "ERROR: identifier_value manquant pour ${key}" >&2
    rm -f "$tmp_out"
    exit 1
  fi

  if [[ -z "$display" ]]; then
    echo "ERROR: display manquant pour ${key}" >&2
    rm -f "$tmp_out"
    exit 1
  fi
  ...
  echo "MEDPLUM_DEVICE_ID_${key}=${device_id}" >> "$tmp_out"
  echo "==> Device ${key} ID: ${device_id}"
done < <(jq -c 'to_entries[]' "$DEVICES_FILE")

mv "$tmp_out" "$IDS_OUT_FILE"
